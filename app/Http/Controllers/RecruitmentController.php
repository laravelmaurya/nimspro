...
class RecruitmentController extends Controller
{
    ...
    if ($request->ajax()) {
        ...
        ->where('nims_recruitment_end_date', '>', Carbon::now())
        ->select([
                    'nims_recruitment_id as id',
                    'nims_recruitment_title as title',
                    'nims_recruitment_number as number',
                    'nims_recruitment_submit_date as submit_date',
                    'nims_recruitment_start_date as start_date',
                    'nims_recruitment_end_date as end_date'
                ])
        ->orderBy('nims_recruitment_number', 'asc')
        ->orderBy('nims_recruitment_id', 'asc')
        ->latest('id','asc');
        // dd($query);
//dd($query->toSql());
            ...
            ->filter(function ($query) use ($request) {
                if ($request->has('search.value')) {
                    ...
                    $query->where(function ($query) use ($search) {
                        $query->where('nims_recruitment_title', 'like', "%{$search}%")
                            ->orWhere('nims_recruitment_number', 'like', "%{$search}%")
                            ->orWhere('nims_recruitment_submit_date', 'like', "%{$search}%")
                            ->orWhere('nims_recruitment_start_date', 'like', "%{$search}%")
                            ->orWhere('nims_recruitment_end_date', 'like', "%{$search}%");
                    });
                }
            })
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            'title' => [
                'required',
                'string',
                'unique:nims_wp_recruitment,nims_recruitment_title',
                'regex:/^[a-zA-Z1-9 ]+$/',
                'min:3',
                'max:50'
            ],
            'number' => [
                ...
                'numeric',
                'digits:10',
                'unique:nims_wp_recruitment,nims_recruitment_number'
            ],
            'start_date' => ['required'],
            'end_date' => ['required'],
        ...
        $path = 'public/uploads/recruitments/' . $directoryDate;
        ...
        // Create a new recruitment and notification data
        $recruitmentData = [
            'nims_add_id' => $add_id,
            'nims_main' => $main_num,
            'nims_recruitment_archive' => $archive,
            'nims_recruitment_title' => $title,
            'nims_recruitment_number' => $number,
            'nims_recruitment_desc' => $description,
            'nims_recruitment_start_date' => $start_date,
            'nims_recruitment_end_date' => $end_date,
            'nims_recruitment_submit_date' => $publish_date,
            'nims_recruitment_doc' => $main_doc,
        ...
        // Add attachment paths to the recruitment data and notification data
        for ($i = 1; $i <= $attachmentCount; $i++) {
            if (isset($uploadedFiles['attachment_' . $i])) {
                $recruitmentData['nims_wp_recruitment_link' . $i] = $uploadedFiles['attachment_' . $i];
        ...
        DB::beginTransaction();
        try {
            // Save the recruitment data
            $recruitment = Recruitment::create($recruitmentData);
            Log::info('Recruitment added: ' . $recruitment->nims_recruitment_id);
            $notificationData['type_id'] = $recruitment->nims_recruitment_id;
            // dd($notificationData);
     ...
     public function edit($id)
    {
        ...
        for($i=1; $i <= 10; $i++){
            $imagelink = 'nims_wp_recruitment_link'.$i;
            $imagelink = $recruitment->$imagelink;
        ...
        $recruitment['additional_attachments'] = $additional_attachments;
        $recruitment['additional_attachments_number'] = $additional_attachments_number;
        // dd($additional_attachments);   
    //    dd($recruitment);
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$recruitment) {
            return redirect()->route('recruitments.index')->with('error', 'Recruitment not found.');
        }       
        ...
        $rules = [
            'title' => [
                'required',
                'string',
                'unique:nims_wp_recruitment,nims_recruitment_title,' . $id .  ',nims_recruitment_id',
                'regex:/^(?!(?>[^.]*\.){2})[A-Za-z0-9 \.]+$/',
                'min:3',
                'max:50'
            ],
            'number' => [
                ...
                'numeric',
                'digits:10',
                // 'unique:nims_wp_recruitment,nims_recruitment_number,' . $id .  ',nims_recruitment_id'
            ],
            'start_date' => ['required'],
            'end_date' => ['required'],
       ...
       if (!$this->dataTamper($title, $h1_title) || !$this->dataTamper($number, $h2_number) || !$this->dataTamperDes($description,$h3)) {
        ...
        // Prepare the updated recruitment and notification data
        $recruitmentData = [
            'nims_main' => $main_num, 
            'nims_recruitment_archive' => $archive,  
            'nims_recruitment_title' => $title,
    ...
    //    dd($notificationData,$na);
        ...
        try {
            ...
            Log::info('Recruitment updated: ' . $recruitment->nims_recruitment_id);
            // dd($recruitment->type_id);
            // dd($notificationData);
            // Update the notification data
            Notification::where('type_id', $recruitment->nims_recruitment_id)->update($notificationData);
            Log::info('Notification updated for recruitment ID and : recruitment number' .$recruitment->nims_recruitment_id. ' and '. $recruitment->nims_recruitment_number);
    ...
    {   
        // dd($request->all());
        $image = 'nims_wp_recruitment_link'.$request->img;
        $recruitment = Recruitment::find($request->id); 
        if ($recruitment) {
            // Get the path of the image file
            $filePath = $recruitment->$image; // Adjust the attribute name according to your model
            // Delete the file from the storage
            if (Storage::exists($filePath)) {                
                unlink(Storage::path($filePath));
                // Delete the image record from the database               
                $recruitment->$image = null;
                $recruitment->save();
    ...
    {   
        ...
        if ($recruitment) {
            // Get the path of the image file
            $filePath = $recruitment->nims_recruitment_doc; // Adjust the attribute name according to your model
            // Delete the file from the storage
            if (Storage::exists($filePath)) {                
                unlink(Storage::path($filePath));
                // Delete the image record from the database               
                $recruitment->nims_recruitment_doc = null;
                $recruitment->save();
    ...
    {
        // dd($id);
       $recruitment = Recruitment::find($id)->delete();
    //    dd($recruitment);
    ...
    public function removeAttachment(Request $request)
    {
        ...
        $recruitment = Recruitment::find($id); 
        $notification = Notification::where(['notifi_number' =>$recruitment->nims_recruitment_number])->first('notifi_id'); 

        // dd($recruitment,$notification);

        if ($recruitment) {
            // Get the path of the image file

            $image = 'nims_wp_recruitment_link'.$attachment_number;
            ...
            if (Storage::exists($filePath)) {                
                unlink(Storage::path($filePath));
                // Delete the image record from the database 
                // dd($recruitment->$image);              
                $recruitment->$image = null;
                $notification->$notifi_image = null;
      
                if($recruitment->save() && $notification->save()){
                        return response()->json([
                            'status'=>'success',
    ...
    public function changeStatusTender(Request $request)
    {
        ...
        // dd($request->id);
        $recruitment = Recruitment::find($request->id);
        $status = ($recruitment->nims_wp_user_status == 1) ? 0: 1; 
        $recruitment->nims_wp_user_status = $status;
        $recruitment->save();
        if($status)
        {
    ...
    public function storeCorrigendum(Request $request)
    {
        ...
        $rules = [
            ...
            'start_date' => ['required'],
            'end_date' => ['required',new CheckedSameDate(Recruitment::class,'nims_recruitment_number','nims_recruitment_end_date','Main Recruitment & Corrigendum')],
            'main_doc' => ['required', 'file', 'max:2048', 'mimes:jpg,jpeg,png,pdf', new NoDoubleExt()]
        ];
        ...
        $applyonline = ($request->applyonline == 1) ? 0: 1; 
    ...
    public function getNumber()
    {
        ...
        $nims_main = 1;
        $nims_recruitment_archive = 0;

        // Perform the query using Eloquent
        $recruitments = Recruitment::where('nims_main', $nims_main)
                         ->where('nims_recruitment_archive', $nims_recruitment_archive)
                         ->where('nims_recruitment_end_date', '>',  Carbon::now())
                         ->orderBy('nims_recruitment_number', 'DESC')
                         ->get(['nims_recruitment_number']);
    ...
    public function listArchive(Request $request)
    {
        $this->authorize('view-page', 'recruitment-list-archive');
            ...
            return view('recruitments.list-archive');
    }
}
