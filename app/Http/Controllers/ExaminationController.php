...
class ExaminationController extends Controller
{
   ...
   //  {
   //      $examinations = Examination::latest('nims_examination_id')->paginate(5);
   //     //  $examinations = Examination:: orderByDesc('nims_examination_id')->paginate(5); // working pagination but searching not all page
   //     //  $examinations = Examination:: orderByDesc('nims_examination_id')->get();
...
{
    $this->authorize('view-page', 'examination-list');
   // dd($request->all());
   if ($request->ajax()) {
       $query = Examination::where('nims_examination_archive', 0)
       ->where('nims_examination_end_date', '>', Carbon::now())
       ->select([
                   'nims_examination_id as id',
                   'nims_examination_title as title',
                   'nims_examination_number as number',
                   'nims_examination_submit_date as submit_date',
                   'nims_examination_start_date as start_date',
                   'nims_examination_end_date as end_date'
               ])
       ->orderBy('nims_examination_number', 'asc')
       ->orderBy('nims_examination_id', 'asc')
       ->latest('id','asc');
       // dd($query);
//dd($query->toSql());
       return DataTables::eloquent($query)
           ->filter(function ($query) use ($request) {
               if ($request->has('search.value')) {
                   ...
                   $query->where(function ($query) use ($search) {
                       $query->where('nims_examination_title', 'like', "%{$search}%")
                           ->orWhere('nims_examination_number', 'like', "%{$search}%")
                           ->orWhere('nims_examination_submit_date', 'like', "%{$search}%")
                           ->orWhere('nims_examination_start_date', 'like', "%{$search}%")
                           ->orWhere('nims_examination_end_date', 'like', "%{$search}%");
                   });
               }
           })
           ...
           ->addColumn('action', function($row){   
                ...
                $editPermission = auth()->user()->can('view-page', 'examination-edit');
   ...
   public function store(Request $request)
   {
       ...
       $rules = [
           'title' => [
               'required',
               'string',
               'unique:nims_wp_examination,nims_examination_title',
               'regex:/^[a-zA-Z1-9 ]+$/',
               'min:3',
               'max:50'
           ],
           'number' => [
               ...
               'numeric',
               'digits:10',
               'unique:nims_wp_examination,nims_examination_number'
           ],
           'start_date' => ['required'],
           'end_date' => ['required'],
       ...
       $path = 'public/uploads/examinations/' . $directoryDate;
       ...
       // Create a new examination and notification data
       $examinationData = [
           'nims_add_id' => $add_id,
           'nims_main' => $main_num,
           'nims_examination_archive' => $archive,
           'nims_examination_title' => $title,
           'nims_examination_number' => $number,
           'nims_examination_desc' => $description,
           'nims_examination_start_date' => $start_date,
           'nims_examination_end_date' => $end_date,
           'nims_examination_submit_date' => $publish_date,
           'nims_examination_doc' => $main_doc,
       ...
       // Add attachment paths to the examination data and notification data
       for ($i = 1; $i <= $attachmentCount; $i++) {
           if (isset($uploadedFiles['attachment_' . $i])) {
               $examinationData['nims_wp_examination_link' . $i] = $uploadedFiles['attachment_' . $i];
       ...
       try {
           ...
           Log::info('Examination added: ' . $examination->nims_examination_id);
           $notificationData['type_id'] = $examination->nims_examination_id;
           // dd($notificationData);
    ...
    public function edit($id)
   {
       ...
       for($i=1; $i <= 10; $i++){
           $imagelink = 'nims_wp_examination_link'.$i;
           $imagelink = $examination->$imagelink;
       ...
       $examination['additional_attachments'] = $additional_attachments;
       $examination['additional_attachments_number'] = $additional_attachments_number;
       // dd($additional_attachments);   
   //    dd($examination);
   ...
   public function update(Request $request, $id)
   {
       ...
       if (!$examination) {
           return redirect()->route('examinations.index')->with('error', 'Examination not found.');
       }       
       ...
       $rules = [
           'title' => [
               'required',
               'string',
               'unique:nims_wp_examination,nims_examination_title,' . $id .  ',nims_examination_id',
               'regex:/^(?!(?>[^.]*\.){2})[A-Za-z0-9 \.]+$/',
               'min:3',
               'max:50'
           ],
           'number' => [
               ...
               'numeric',
               'digits:10',
               // 'unique:nims_wp_examination,nims_examination_number,' . $id .  ',nims_examination_id'
           ],
           'start_date' => ['required'],
           'end_date' => ['required'],
      ...
      if (!$this->dataTamper($title, $h1_title) || !$this->dataTamper($number, $h2_number) || !$this->dataTamperDes($description,$h3)) {
       ...
       // Upload main document
       $main_doc = $examination->nims_examination_doc; // default to existing main document
       $file = $request->file('main_doc');
       if ($file) {
           $main_doc = $this->uploadAndSanitizeFile(14,$request->number, $path, $file);
           Log::info('Main document uploaded: ' . $main_doc);
       ...
       // Prepare the updated examination and notification data
       $examinationData = [
           'nims_main' => $main_num, 
           'nims_examination_archive' => $archive,  
           'nims_examination_title' => $title,
   ...
   //    dd($notificationData,$na);
       ...
       try {
           ...
           $examination->update($examinationData);
           Log::info('Examination updated: ' . $examination->nims_examination_id);
           // dd($examination->type_id);
           // dd($notificationData);
           // Update the notification data
           Notification::where('type_id', $examination->nims_examination_id)->update($notificationData);
           Log::info('Notification updated for examination ID and : examination number' .$examination->nims_examination_id. ' and '. $examination->nims_examination_number);
   ...
   {   
       // dd($request->all());
       $image = 'nims_wp_examination_link'.$request->img;
       $examination = Examination::find($request->id); 
       if ($examination) {
           // Get the path of the image file
           $filePath = $examination->$image; // Adjust the attribute name according to your model
           // Delete the file from the storage
           if (Storage::exists($filePath)) {                
               unlink(Storage::path($filePath));
               // Delete the image record from the database               
               $examination->$image = null;
               $examination->save();
   ...
   {   
       ...
       if ($examination) {
           // Get the path of the image file
           $filePath = $examination->nims_examination_doc; // Adjust the attribute name according to your model
           // Delete the file from the storage
           if (Storage::exists($filePath)) {                
               unlink(Storage::path($filePath));
               // Delete the image record from the database               
               $examination->nims_examination_doc = null;
               $examination->save();
   ...
   public function removeAttachment(Request $request)
   {
       ...
       $examination = Examination::find($id); 
       $notification = Notification::where(['notifi_number' =>$examination->nims_examination_number])->first('notifi_id'); 

       // dd($examination,$notification);

       if ($examination) {
           // Get the path of the image file

           $image = 'nims_wp_examination_link'.$attachment_number;
           ...
           if (Storage::exists($filePath)) {                
               unlink(Storage::path($filePath));
               // Delete the image record from the database 
               // dd($examination->$image);              
               $examination->$image = null;
               $notification->$notifi_image = null;
     
               if($examination->save() && $notification->save()){
                       return response()->json([
                           'status'=>'success',
   ...
   public function changeStatusEvent(Request $request)
   {
       ...
       // dd($request->id);
       $examination = Examination::find($request->id);
       $status = ($examination->nims_wp_user_status == 1) ? 0: 1; 
       $examination->nims_wp_user_status = $status;
       $examination->save();
       if($status)
       {
   ...
   public function storeCorrigendum(Request $request)
   {
       ...
       $rules = [
           ...
           'start_date' => ['required'],
           'end_date' => ['required',new CheckedSameDate(Examination::class,'nims_examination_number','nims_examination_end_date','Main Examination & Corrigendum')],
           'main_doc' => ['required', 'file', 'max:2048', 'mimes:jpg,jpeg,png,pdf', new NoDoubleExt()]
       ];
   ...
   public function getNumber()
   {
       ...
       $nims_main = 1;
       $nims_examination_archive = 0;

       // Perform the query using Eloquent
       $examinations = Examination::where('nims_main', $nims_main)
                        ->where('nims_examination_archive', $nims_examination_archive)
                        ->where('nims_examination_end_date', '>',  Carbon::now())
                        ->orderBy('nims_examination_number', 'DESC')
                        ->get(['nims_examination_number']);
   ...
   {
    $this->authorize('view-page', 'examination-list-archive');
           if ($request->ajax()) {
               $query = Examination::where('nims_examination_archive', 1)
                           ->orWhere('nims_examination_end_date', '<',  Carbon::now())
                   ->select([
                       'nims_examination_id as id',
                       'nims_examination_title as title',
                       'nims_examination_number as number',
               ...
               return DataTables::eloquent($query)
                   ...
                   ->addIndexColumn()
                   ->addColumn('action', function($row){
                    $editPermission = auth()->user()->can('view-page', 'examination-edit-archive');
           ...
           return view('examinations.list-archive');
   }
}
